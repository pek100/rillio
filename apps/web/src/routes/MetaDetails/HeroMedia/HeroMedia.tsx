                ...
                count > 1 ?
                    <React.Fragment>
                        <IconButton
                            aria-label={'Previous'}
                            onClick={() => go(-1)}
                            className="absolute left-2.5 top-1/2 z-[2] size-9 -translate-y-1/2 bg-glass-chrome opacity-0 backdrop-blur-(--glass-blur) transition group-hover:opacity-100 hover:bg-glass-panel"
                        >
                        ...
                        <IconButton
                            aria-label={'Next'}
                            onClick={() => go(1)}
                            className="absolute right-2.5 top-1/2 z-[2] size-9 -translate-y-1/2 bg-glass-chrome opacity-0 backdrop-blur-(--glass-blur) transition group-hover:opacity-100 hover:bg-glass-panel"
                        >
                            <ChevronRight className="size-4 text-fg" />
                        </IconButton>
                        <div className="absolute bottom-2.5 left-1/2 z-[2] flex -translate-x-1/2 gap-1.5 rounded-full bg-glass-chrome px-2.5 py-1.5 backdrop-blur-(--glass-blur)">
                            {slides.map((slide, index) => (
                                <button
                                    key={index}
                                    type={'button'}
